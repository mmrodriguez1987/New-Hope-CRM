...
class CreatePersonhistoryTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('personhistory', function (Blueprint $table) {
            ...
            $table->integer('resp_leader_id')->unsigned()->reference('id')->on('persona');
            $table->integer('invl_person_id')->unsigned()->reference('id')->on('persona');
            $table->integer('event_id')->unsigned()->reference('id')->on('evento');
