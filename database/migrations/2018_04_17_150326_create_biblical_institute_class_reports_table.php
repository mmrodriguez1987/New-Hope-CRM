...
class CreateBiblicalInstituteClassReportsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('biblical_institute_class_reports', function (Blueprint $table) {
            $table->increments('id');
            $table->date('date');
            $table->integer('leader_respons_id')->refrence('id')->on('persons');
            $table->integer('srv_ppt_id')->refrence('id')->on('persons');
            $table->integer('srv_snd_id')->refrence('id')->on('persons');
            $table->integer('srv_vta_id')->refrence('id')->on('persons');
            $table->double('sale_amount', 8, 2);
            $table->mediumtext('teaching_taught');
            $table->datetime('hour_init');
            $table->datetime('hour_end');
            $table->integer('teacher_1_id')->refrence('id')->on('persons');
            $table->integer('teacher_2_id')->refrence('id')->on('persons');  
